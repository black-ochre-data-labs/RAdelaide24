...
- Read QC is often performed using `FastQC` or `fastp`
    + `fastp` will also remove low quality bases/reads
    + Standalone trimming/filtering tools (`cutadapt`, `AdapterRemoval` etc)
- MultiQC provides good summaries of QC reports across a dataset
    + `ngsReports` enables import into R for custom plots/tables

. . .

- Alignment to a reference genome needs a splice-aware aligner
    + `STAR` is the most common followed by `hisat2` & `bowtie2`

## What Tools

- Counting alignments usually uses `featureCounts`, `RSEM` or `htseq2`
    + Needs a GTF for counting alignments
    
. . .

- Alignments to a reference *transcriptome* only returns counts
    + Most common tools are `salmon` or `kallisto`
    + Also returns uncertainty for alignments
