...
- For the rest of this session we'll look at some SNPs
...
- We have `r nrow(snps)` participants with `r ncol(snps) - 1` SNP genotypes
...
- Perform Fisher's Exact Test on each SNP locus
    + Tests for association between genotype and population
...
- If we pass to `summarise()` we can count these
...
snps %>% 
  ...
  summarise(
    n = dplyr::n(), .by = c(Population, locus, genotype)
  ) %>% 
  pivot_wider(names_from = "Population", values_from = "n", values_fill = 0) %>% 
  mutate(
    locus = factor(
      locus, levels = str_sort(unique(locus), numeric = TRUE)
    )
  ) %>% 
  nest(df = c(genotype, Control, Treat)) %>% 
  mutate(
    ft = lapply(df, \(x) fisher.test(x[,c("Control", "Treat")])),
    p = map_dbl(ft, \(x) x$p.value),
    adj_p = p.adjust(p, "fdr")
  ) %>% 
  arrange(p)
## This uses 3x2 tables so there's no odds ratio
```
...
snps %>% 
  ...
  dplyr::select(Population, locus, A, B) %>% 
  nest(df = c(Population, A, B)) %>% 
  mutate(
    ft = lapply(df, \(x) fisher.test(x[,c("A", "B")])),
    OR = map_dbl(ft, \(x) x$estimate),
    Control = map_int(df, \(x) dplyr::filter(x, Population == "Control")[["B"]]),
    Treat = map_int(df, \(x) dplyr::filter(x, Population == "Treat")[["B"]]),
    p = map_dbl(ft, \(x) x$p.value),
    adj_p = p.adjust(p, "fdr")
  ) %>% 
  arrange(p)
